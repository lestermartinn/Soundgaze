...
import { fetchDescription, type SongPoint } from "../lib/api";
...
export default function ExplorePage() {
  ...
  const fetchDescriptionForSelected = useCallback(async () => {
    const song = selection.selectedSong;
    if (!song) return;
    if (song.description || song.isDescriptionLoading) return;

    const point = allPoints.find((p) => p.track_id === song.id);
    const name = song.title ?? point?.name ?? "";
    const artist = song.artist ?? point?.artist ?? "";
    const genre = point?.genre ?? "";

    selection.setSelectedSong((prev) => prev ? { ...prev, isDescriptionLoading: true } : prev);
    try {
      const { description } = await fetchDescription(name, artist, genre);
      selection.setSelectedSong((prev) => prev ? { ...prev, description, isDescriptionLoading: false } : prev);
    } catch {
      selection.setSelectedSong((prev) => prev ? { ...prev, isDescriptionLoading: false } : prev);
    }
  }, [selection.selectedSong, selection.setSelectedSong, allPoints]);
